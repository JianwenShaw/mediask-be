...
public class GreedyLocalSearchSolver implements ScheduleSolver {
    ...
    public ScheduleSolution solve(ScheduleProblem problem) {
        ...
        try {
            // 第一阶段：贪婪初始化
            SolverContext currentContext = greedyInitialize(problem);
            ...
            SolverContext bestContext = currentContext.copy();

            while (iterations < MAX_ITERATIONS) {
                iterations++;

                // 生成邻居解
                SolverContext neighbor = generateNeighbor(currentContext, problem);
     ...
     * 贪婪初始化
     */
    private SolverContext greedyInitialize(ScheduleProblem problem) {
     ...
     * 生成邻居解（交换两个排班位置的医生）
     */
    private SolverContext generateNeighbor(SolverContext current, ScheduleProblem problem) {
        SolverContext neighbor = current.copy();
